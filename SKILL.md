...
4. **If the person asks for an editable `.docx`,** the path is [`ooxmlkit`](https://github.com/Carlos-Padilla-Bravo/ooxmlkit), a separate published library that typesets Word from the same decisions. Optional, and not part of this skill. Do not write the program from scratch: its `ejemplo/manual.py` already typesets a complete manual for Ignacia, the same person shipped here, and `ejemplo/ignacia.py` holds nothing but her decisions. Copy that split, put the person's ten colour roles and three families where Ignacia's are, and adapt the prose. **Check the two requirements before offering it**, because neither is negotiable and the HTML deliverable has neither: closing the document needs Windows with Microsoft Word, and the chosen fonts must be installed on that machine or Word substitutes silently and the manual contradicts what it declares about itself. On macOS or Linux, say so plainly and stop at the HTML, which already prints the formal paginated PDF anywhere.
